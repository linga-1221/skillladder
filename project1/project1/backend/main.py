...
async def get_all_jobs():
    jobs = list(JOBS_DB)
    try:
        firebase_jobs = await firebase_service.get_jobs()
        if firebase_jobs:
            # Normalize Firebase shape to frontend-consumable job objects
        ...
        print(f"Error loading Firebase jobs in get_all_jobs: {e}")

    # Deduplicate by id if present; otherwise by title+company+posted_by
    dedup = {}
    for job in jobs:
        key = str(job.get("id")) if job.get("id") is not None else f"{job.get('title')}|{job.get('company')}|{job.get('posted_by')}"
        dedup[key] = job

    return {"jobs": list(dedup.values())}
...
async def get_job_applications(job_id: int = None, user_email: str = None):
    try:
        ...
        if firebase_apps:
            return {"applications": firebase_apps}
    except Exception as e:
...
async def post_job(request: Request):
    try:
        ...
        try:
            await firebase_service.create_job({
                "title": new_job["title"],
...
async def get_jobs_by_provider(posted_by: str):
    ...
    try:
        firebase_jobs = await firebase_service.get_jobs({"posted_by": posted_by})
        if firebase_jobs:
            for job in firebase_jobs:
                jobs.append({
                    "id": job.get("id"),
        ...
        print(f"Error loading provider jobs from Firebase: {e}")
