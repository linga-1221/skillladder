  ...
  useEffect(() => {
    ...
    const loadJobs = async () => {
      ...
      try {
        ...
        // Merge backend jobs with sample jobs (avoid duplicate IDs).
        const sampleById = new Map(sampleJobs.map((j) => [j.id, j]));
        backendJobs.forEach((j) => sampleById.set(j.id, j));
        setJobs(Array.from(sampleById.values()));
      } catch (_) {
        // Fallback to sample data if backend isn’t reachable.
        setJobs(sampleJobs);
      } finally {
        setLoading(false);
      }
