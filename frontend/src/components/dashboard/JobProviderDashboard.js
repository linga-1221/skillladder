...
export default function EmployeeDashboard({ user }) {
  ...
  const loadEmployeeData = async () => {
    ...
    try {
      ...
      const jobsResponse = await fetch(`http://localhost:8000/get_jobs_by_employee/?posted_by=${user.email}`);
      if (jobsResponse.ok) {
      ...
      // Load analytics
      const analyticsResponse = await fetch(`http://localhost:8000/get_employee_analytics/?posted_by=${user.email}`);
      ...
      const applicationsResponse = await fetch(`http://localhost:8000/get_job_applications/`);
  ...
  const handleJobSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/post_job/', {
        method: 'POST',
  ...
  const handleDeleteJob = async (jobId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/delete_job/${jobId}?posted_by=${user.email}`, {
        method: 'DELETE'
      });
